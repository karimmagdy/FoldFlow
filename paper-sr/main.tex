...
addition rather than the folding interpretation. On WikiText-2, the
causally-safe FoldFlow-LM reduces perplexity by $4.1\%$ over a
matched Transformer++ baseline ($577.6$ vs.\ $602.3$); a per-token
causal gate is marginally better ($-4.5\%$) and Talking-Heads does
not improve. An earlier non-causal formulation reported a $14.4\%$
reduction; we trace and correct that result. The finding is
modality-dependent: head-level gating helps a small but reproducible
amount, the specific energy framing is not critical, and not every
folding principle transfers across modalities.
\end{abstract}
...
WikiText-2, the post-fix causally-safe FoldFlow-LM (50.2~M) achieves
$577.6 \pm 1.9$ perplexity versus $602.3 \pm 4.2$ for the default
Transformer++ (44.8~M)---a $4.1\%$ relative reduction. An earlier
formulation of the energy gate pooled context non-causally and
reported $512.5 \pm 3.6$ ($14.4\%$ reduction); we trace this gap to
teacher-forced future-token leakage and replace the gate with a
causal cumulative-mean variant, restoring autoregressive correctness
(Methods, ``Causal energy gating (W1 fix)''; Table~\ref{tab:lm_attn_variants}).
...
module. Full derivations are given in the Methods section.
...
gains. The reviewer asked the directly probative question: does the
...
over 3 seeds. All FoldFlow rows share the same weak encoder
...
rows.} The ``Topology MHA only'' row addresses reviewer W5: the
...
removed. Published baselines use standard encoders and are shown
for context, not as head-to-head comparisons.}
\label{tab:ablation}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
...
\midrule
\multicolumn{3}{@{}l}{\emph{Reviewer W5 control (no folding dynamics; encoder + 1 MHA layer once):}} \\
...
transformer,\cite{ba2016layer} GELU activations, tied embeddings)
...
\textbf{Pre-fix and post-fix numbers.} An earlier version of this
paper reported a $14.4\%$ relative perplexity reduction
($512.5 \pm 3.6$ for FoldFlow-LM versus $598.4 \pm 3.9$ for
Transformer++), under an energy-gate formulation that pooled the
context globally---i.e.\ the per-head gate at every position read
every token of the same training sequence, including positions
strictly in the future. Although the multi-head attention itself
was masked, the gate was a function of the global mean and
therefore depended on tokens at $t' > t$ during teacher forcing.
We trace this leakage to a single $\texttt{x.mean(dim=1)}$ in the
energy-gating implementation and replace it with a causal cumulative
mean (Methods, ``Causal energy gating (W1 fix)''). All numbers
reported below are from the post-fix code, re-run with 5 seeds and
17 epochs on a CUDA / GTX 1080 Ti backend; the pre-fix numbers are
retained in Table~\ref{tab:lm_attn_variants} for reference and
clearly labelled as not comparable.

Under the corrected gate, FoldFlow-LM achieves a mean
...
(paired $t = 13.6$, $p < 10^{-3}$ across 5 seeds), substantially
smaller than the leakage-confounded $14.4\%$ but still
...
baseline named explicitly by the reviewer---attains $606.1 \pm 6.1$,
...
(Methods, ``Causal energy gating (W1 fix)''), re-run with 5 seeds
...
\begin{figure}[t]
  ...
  not improve over the baseline. The figure originally reported a
  $14.4\%$ reduction under a non-causal cumulative-mean gate that
  read future tokens during teacher forcing
  (Methods, ``Causal energy gating (W1 fix)''); those numbers are
  retained in the lower block of Table~\ref{tab:lm} for reference.
...
future-token leakage (Methods, ``Causal energy gating (W1 fix)''). We
...
baseline named explicitly by the reviewer---does not improve over
...
sensitivity sweep at $d{=}560$ (the reviewer's W6 ask), reported
...
attention pattern; \emph{IBiT}\cite{ibit2023} more generally
...
early steps the noise facilitates broad exploration, at late steps
...
causal, and Talking-Heads); see ``Causal energy gating (W1 fix)''
...
\paragraph{Causal energy gating (W1 fix).}
...
head-mixing baseline that the reviewer-named gating literature
...
CIFAR-10 and CIFAR-100 are distributed by the University of
Toronto.\cite{krizhevsky2009cifar} WikiText-2 is available from
the Salesforce Research pointer-sentinel
repository\cite{merity2017pointer}. We used the standard Hugging
Face distribution for all three datasets. No new experimental data
were generated. Pre-processing scripts, tokenisation details (GPT-2
tokeniser, 50{,}257 tokens), and per-seed numerical results are
...
sufficient to reproduce every table and figure in this paper. Experiments were
...
M4) and by Google Colab Pro. No external funding supported this
work.
