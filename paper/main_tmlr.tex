% FoldFlow: A Systematic Study of Bio-Inspired Inductive Biases for Neural Architecture Design
% FoldFlow paper — TMLR submission version
%
% Options: [accepted] for published version, [preprint] for arXiv, default = submission
...
% ---- TODO command (disabled for submission) ----
\newcommand{\todo}[1]{}
...
Langevin dynamics on a learned energy landscape, data-dependent
...
\emph{which principle helps depends on the modality, and several
help not at all}:
\textbf{(i)}~on CIFAR-10 classification (1.48M parameters, weak
encoder), adaptive topology provides the largest individual gain
(+1.3~pp); the full model reaches 83.0\% versus an 81.6\% baseline
(a small +1.4~pp gain) and 85.7\% with extended training;
\textbf{(ii)}~on WikiText-2 language modeling, energy-gated attention
is essentially the only driver, reducing perplexity by 14.4\%
relative to the default Transformer++ baseline (512.5 vs.\ 598.4,
5 seeds; the parameter-matched 50.9M Transformer++ control reaches
$593.3$~PPL, so the FoldFlow advantage is not a parameter-count
artifact), while the Langevin refinement module---the most direct
analog of the folding process itself---has \emph{no measurable
effect} (512.5 with and without).  FoldFlow~LM also trains
2.3$\times$ faster than the baseline in tokens per second.  Rather
than claiming that the principles of protein folding transfer to
neural architecture design, we treat this as a systematic empirical
study of bio-inspired inductive biases and report the heterogeneous
outcome honestly.
...
modalities}.  On CIFAR-10, adaptive topology~(C2)---multi-head attention
...
for most of the 14.4\% perplexity reduction.  This modality-dependent
...
\begin{enumerate}
  \item A mapping from five protein folding characteristics to
    differentiable neural modules, applicable to both vision
    (particle-based) and language (Transformer-based) architectures
    (Section~\ref{sec:method}).  We frame this as a \emph{systematic
    empirical study} of bio-inspired inductive biases rather than a
    claim that the underlying principles transfer wholesale: some of
    the modules help, others have no measurable effect, and the
    pattern depends on the modality.
  \item A systematic ablation (7 configurations $\times$ 3 seeds) on
    CIFAR-10 that disentangles per-component contributions
    (Section~\ref{sec:ablation}).  The full model improves over the
    weak-encoder baseline by a small but measurable +1.4~pp
    (83.0\% vs.\ 81.6\%); we do not claim a dramatic gain.
  \item A controlled language modeling study on WikiText-2 in which
    FoldFlow~LM reaches 14.4\% lower perplexity than the
    Transformer++ baseline at the same hyperparameters
    (Section~\ref{sec:lm}).  Component ablation attributes essentially
    all of this gain to \emph{energy-gated attention}, while the
    Langevin refinement module has \emph{no measurable effect}
    (full vs.\ no-Langevin: 512.5 vs.\ 512.5~PPL).  FoldFlow~LM has
    50.2M parameters versus 44.8M for the default Transformer++
    baseline (a 12\% confound); a parameter-matched Transformer++
    (50.9M, $593.3 \pm 1.1$~PPL; Section~\ref{sec:param_matched})
    closes most of this gap and is the comparison we foreground when
    discussing the 14.4\% number.
  \item A cross-domain analysis showing that the dominant module is
    \emph{modality-dependent}: adaptive topology helps in vision,
    energy-gated attention helps in language, and Langevin dynamics
    helps in neither LM nor (substantially) vision.  We treat this
    heterogeneous outcome as the honest finding of the paper rather
    than packaging it as a uniform success story
...
We do not claim novelty for any individual component; each maps to known computational mechanisms (Section~\ref{sec:method}). Our contribution is the systematic mapping from biological principles to neural architecture design and the empirical finding that their relative importance depends on the target modality.
...
C2 & Adaptive Topology & Contact formation \& rearrangement
...
Protein contact maps depend on the amino acid sequence and its
environment.  We model this with multi-head self-attention over the
particle set, computed \emph{once} after initialization
(Algorithm~\ref{alg:foldflow}, line~3) to establish data-dependent
connectivity before the dynamics phase begins:
\begin{equation}
  \bz \leftarrow \bz + \text{MHA}(\text{LN}(\bz)).
\end{equation}
Unlike iteratively updated topology, this learned topology is fixed
for the duration of the Langevin dynamics, analogous to how the
native contact map of a protein is determined by its sequence rather
than recomputed at each folding step.
...
This mechanism is closely related to Feature-wise Linear Modulation
(FiLM)~\citep{perez2018film}, which applies learned affine
transformations conditioned on an auxiliary input.  Our variant differs
in that the conditioning signal $\bc$ derives from the same input
encoder (rather than a separate modality) and uses a Softplus-shifted
scale to ensure stability during Langevin dynamics, but the core
principle---context-dependent scale and shift---is shared.
...
\STATE $\bz^{(0)} \leftarrow \bz^{(0)} + \text{MHA}(\text{LN}(\bz^{(0)}))$ \COMMENT{C2: Adaptive topology (computed once)}
...
single Apple M-series chip (MPS backend); full configurations and per-seed results are
...
baseline encoder, we test five hypotheses about which protein folding principles transfer, finding that the relative importance is domain-dependent: topology (C2) dominates in vision while energy gating dominates in language modeling. Each component is added incrementally to C1 because C3--C5 modulate the dynamics that C1 provides.  All configurations share the same architecture
...
Adaptive topology~(C2) provides the largest single gain ($+1.3$~pp over
baseline), consistent with its role in enabling input-conditioned
information routing---analogous to how the native contact map is the
primary determinant of folding rate~\citep{dill2012protein}.  The full
model achieves 83.0\% ($\pm$ 0.4), a +1.4~pp gain over the baseline,
confirming that the five components work in concert.
We emphasize upfront that this is a \textbf{small gain in absolute
terms} (83.0\% vs.\ 81.6\%): we do not claim a dramatic CIFAR-10
improvement, and the result should be read as evidence that
the FoldFlow modules add measurable, not transformative, value on top
of an intentionally weak encoder.

Statistical significance was assessed via two-sample t-tests across seeds. The full model significantly outperforms the baseline ($p < 0.001$), as does the +energy+topology variant ($p < 0.001$). Individual components C1 (+energy alone, $p = 0.28$), C3 (+cooperativity, $p = 0.053$), C4 (+chaperone, $p = 0.20$), and C5 (+environmental sensitivity, $p = 0.32$) do not reach significance individually, confirming that the benefit arises from their combination rather than any single component.
...
\begin{figure}[t]
  ...
  \caption{Ablation study on CIFAR-10.  Each bar shows mean test accuracy
    ($\pm$ std) over 3 seeds.  Adaptive topology~(C2) contributes the
...
warmup~\citep{loshchilov2017decoupled}, up to 17 epochs with early stopping, and 5 seeds.
FoldFlow~LM adds energy-gated attention, chaperone correction within
each block, and Langevin refinement ($K{=}3$) at inference.

\begin{table}[t]
\centering
\caption{WikiText-2 perplexity comparison.  Mean $\pm$ std over 5 seeds,
  early stopping (best validation at epoch 6--7).
...
\midrule
\multicolumn{3}{@{}l}{\emph{Our models (identical hyperparameters, early stopping):}} \\
Transformer++          & 44.8M & $598.4 \pm 3.9$ \\
\midrule
FoldFlow w/o energy gate & 50.2M & $573.4 \pm 1.9$ \\
FoldFlow w/o chaperone   & 50.2M & $517.4 \pm 3.5$ \\
FoldFlow w/o Langevin    & 50.2M & $512.5 \pm 3.5$ \\
FoldFlow LM (full)       & 50.2M & $\mathbf{512.5 \pm 3.6}$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Results.}
FoldFlow~LM achieves a perplexity of 512.5, a \textbf{14.4\% relative
improvement} over the Transformer++ baseline (Table~\ref{tab:lm},
Figure~\ref{fig:wikitext2}).  All models converge at epoch 6--7 (early
stopping at epoch 16--17), with 5 seeds per configuration.
On WikiText-2, the full FoldFlow model achieves 14.4\% lower perplexity than the Transformer++ baseline ($p < 0.001$, paired t-test, 5 seeds). The energy gate is the most impactful individual component: removing it degrades perplexity by 60.9 points ($p < 0.001$).
\textbf{Two caveats are foregrounded here rather than buried.}  First,
this 14.4\% comparison is between a 50.2M FoldFlow model and a 44.8M
Transformer++ baseline---a 12\% parameter mismatch.  The
parameter-matched control (Transformer++ at $d{=}560$, 50.9M params,
$593.3 \pm 1.1$~PPL; Section~\ref{sec:param_matched},
Table~\ref{tab:lm_matched}) is the appropriate reference, and we cite it
each time we report the 14.4\% figure.  Second, the Langevin refinement
module~(C1) has \emph{no measurable effect} on WikiText-2 perplexity:
removing it gives 512.5 versus 512.5 for the full model
(Table~\ref{tab:lm}).  All of FoldFlow~LM's measurable gain over the
Transformer++ baseline is attributable to energy-gated attention
(and to a smaller extent the chaperone), not to the Langevin component
that motivates the protein-folding analogy.

We emphasize that our absolute perplexity
values are high compared to published results (AWD-LSTM: 65.8,
Transformer-XL: 54.5) because we deliberately use limited training
...
largest degradation ($573.4$ vs.\ $512.5$, $+60.9$~PPL), indicating
that context-dependent head gating is the primary driver.  Removing
chaperone correction has a smaller effect
($517.4$), suggesting it provides
a complementary but secondary benefit at this scale.
Notably, the model without Langevin achieves identical perplexity to
the full model (512.5 vs.\ 512.5), indicating that Langevin refinement
has no measurable effect in the language modeling domain; we discuss this
...
\begin{figure}[t]
  ...
  \caption{WikiText-2 perplexity comparison.  FoldFlow~LM reduces
    perplexity by 14.4\% relative to Transformer++.  The component
    ...
    ($d_\text{model}{=}512$, 6 layers, early stopping, 5 seeds).}
  \label{fig:wikitext2}
\end{figure}
...
\caption{Training efficiency on WikiText-2.  Throughput in tokens/second
  (higher is better).  Measured on Apple M-series chip (MPS backend), averaged over
  epochs 2--17 across 5 seeds.}
...
FoldFlow w/o energy gate & 3,489 & 809 & 1.04$\times$\footnotemark \\
FoldFlow w/o chaperone  & 7,801 & 378 & 2.33$\times$ \\
FoldFlow w/o Langevin   & 8,174 & 292 & 2.44$\times$ \\
FoldFlow LM (full)      & 7,575 & 396 & 2.26$\times$ \\
\bottomrule
\end{tabular}
\end{table}

\footnotetext{The ``w/o energy gate'' variant is unexpectedly slower than
the full model (809s vs.\ 396s per epoch) despite removing a component.
Without the energy gate, the model falls back to \texttt{nn.MultiheadAttention},
which incurs the same MPS overhead as Transformer++.  The other FoldFlow
variants retain the manual QKV projection path that the energy gate
requires, and therefore benefit from its faster einsum-based implementation.}

Unexpectedly, FoldFlow~LM trains \textbf{2.26$\times$ faster} than
...
the MPS backend.  The Langevin refinement (applied only at inference)
adds negligible training cost.  We note that this speed advantage is
...
--- & Energy Gate (LM only) & --- & +60.9 \\
\bottomrule
\end{tabular}
\end{table}

The pattern is striking: in vision, \textbf{adaptive topology~(C2)}
...
attention} dominates, accounting for $60.9/85.9 = 71\%$ of the total
improvement of FoldFlow~LM over the default Transformer++ baseline (the
14.4\% perplexity reduction discussed in Section~\ref{sec:lm} and
controlled for parameter count via the wider Transformer++ in
Section~\ref{sec:param_matched}, Table~\ref{tab:lm_matched}).  The
Langevin component~(C1) has \textbf{no measurable effect} on
WikiText-2 perplexity (512.5 with and without; the difference is
exactly zero in the mean), and only a small effect on CIFAR-10.  The
honest reading is that the Langevin module---arguably the most
direct mapping of the protein-folding metaphor onto neural
computation---does not pay off on either of the two tasks we study.
Figure~\ref{fig:cross_domain} visualizes this modality-dependent and
component-dependent pattern.
...
\begin{figure}[t]
  ...
  \caption{Cross-domain component importance.  \textbf{Left:} CIFAR-10
    ...
    component (higher = component helps more).  Adaptive topology
...
Model & Parameters & Perplexity ($\downarrow$) \\
\midrule
Transformer++ ($d{=}512$) & 44.8M & $598.4 \pm 3.9$ \\
Transformer++ ($d{=}560$) & 50.9M & $593.3 \pm 1.1$ \\
FoldFlow LM ($d{=}512$)   & 50.2M & $512.5 \pm 3.6$ \\
\bottomrule
\end{tabular}
\end{table}
...
$d_\text{model}{=}560$ (50.9M parameters, 10 epochs, 3 seeds;
Table~\ref{tab:lm_matched}).  The wider Transformer++
achieves $593.3 \pm 1.1$ perplexity---nearly identical to the
original $d{=}512$ model ($598.4 \pm 3.9$) despite 14\% more
parameters.  FoldFlow's advantage ($512.5 \pm 3.6$) therefore cannot
...
With converged training (5 seeds, early stopping at epoch 16--17,
best validation at epoch 6--7), the FoldFlow model without Langevin
achieves identical perplexity to the full model (512.5 vs.\ 512.5~PPL),
confirming that Langevin refinement has \textbf{no measurable effect}
in the language modeling domain.  We hypothesize two factors: (1)~the
energy head that drives Langevin refinement does not receive sufficient
gradient signal in the autoregressive setting, and (2)~the
detach-and-reattach loop breaks gradient flow, preventing the energy
head from learning useful refinement directions.  In the vision
setting, Langevin operates on a shared particle set with pooled
readout, which is more tolerant of per-step noise and provides a
clearer energy landscape for refinement.
...
(2)~adaptive topology (attention over particles) is valuable when the
...
discovered a surprising modality dependence: adaptive topology dominates
...
The full FoldFlow~LM achieves 14.4\% lower perplexity than the
default Transformer++ on WikiText-2 (50.2M vs.\ 44.8M; the
parameter-matched 50.9M Transformer++ control reaches
$593.3 \pm 1.1$~PPL, leaving the FoldFlow advantage at 13.6\% rather
than from raw capacity), while training 2.3$\times$ faster.  The
CIFAR-10 ablation shows a smaller +1.4~pp improvement (83.0\% vs.\
81.6\%) over a deliberately weak encoder.  We do not claim that
protein folding principles transfer to neural architecture design as a
general rule: Langevin refinement, the most direct neural analog of
the folding process itself, has no measurable effect in the LM setting
and only a modest one in vision.  The honest takeaway is that a few
specific bio-inspired modules---energy-gated attention in language,
adaptive topology in vision---help on the tasks we study, while others
do not, and the only way to tell which is which is component-level
ablation.
...
All experiments were run on a single Apple M-series chip (MPS backend).  Full
...
Code is available at \url{https://github.com/karimmagdy/FoldFlow}.

% ====================================================================
\section*{Broader Impact}
This work explores bio-inspired inductive biases for neural architecture design. The principles studied (energy minimization, topology, etc.) are general-purpose and do not raise specific ethical concerns beyond those common to neural network research. The computational cost of our experiments is modest (single-GPU training), and all code and configurations will be released to support reproducibility.
