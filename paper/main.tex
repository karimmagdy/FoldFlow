...
the Transformer++ baseline (525.2 vs.\ 598.7).
FoldFlow~LM achieves this improvement while training 2.3$\times$ faster
...
\begin{enumerate}
  ...
  \item A controlled language modeling study showing FoldFlow~LM
    ...
    showing a 2.3$\times$ training speed advantage
    (Section~\ref{sec:lm}).
...
original $d{=}512$ model ($598.7 \pm 0.5$) despite 14\% more
...
Transformer++ on WikiText-2 while training 2.3$\times$ faster, and
...
This work explores bio-inspired inductive biases for neural architecture design. The principles studied (energy minimization, topology, etc.) are general-purpose and do not raise specific ethical concerns beyond those common to neural network research. The computational cost of our experiments is modest (single-GPU training), and we release all code and configurations to support reproducibility.
